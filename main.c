...
int main(void)
{
  /* USER CODE BEGIN 1 */
	...
	int octvMulti = 1;
	enum eOctaveStatus { first, second, third, fourth }  octaveStatus = first;
  /* USER CODE END 1 */
  ...
  while (1)
  {
	...
	//up and down octave
	if(checkButtonPress(GPIOB, b14) == 1 && octaveStatus == first){
		octvMulti = 2;
		octaveStatus = second;
	}
	if(checkButtonPress(GPIOB, b14) == 1 && octaveStatus == second){
		octvMulti = 3;
		octaveStatus = third;
	}
	if(checkButtonPress(GPIOB, b14) == 1 && octaveStatus == third){
		octvMulti = 4;
		octaveStatus = fourth;
	}
	if(checkButtonPress(GPIOB, b15) == 1 && octaveStatus == fourth){
		octvMulti = 3;
		octaveStatus = third;
	}
	if(checkButtonPress(GPIOB, b15) == 1 && octaveStatus == third){
		octvMulti = 2;
		octaveStatus = second;
	}
	if(checkButtonPress(GPIOB, b15) == 1 && octaveStatus == second){
		octvMulti = 1;
		octaveStatus = first;
	}
	//C
	if(checkButtonPress(GPIOB, b4) == 1){
		changeFrequency(octvMulti * 130.0f);
	}
	//D
	if(checkButtonPress(GPIOB, b5) == 1){
		changeFrequency(octvMulti * 146.0f);
	}
	//E
	if(checkButtonPress(GPIOB, b7) == 1){
		changeFrequency(octvMulti * 164.0f);
	}
	//F
	if(checkButtonPress(GPIOB, b8) == 1){
		changeFrequency(octvMulti * 174.0f);
	}
	//G
	if(checkButtonPress(GPIOB, b11) == 1){
		changeFrequency(octvMulti * 196.0f);
	}
	//A
	if(checkButtonPress(GPIOB, b12) == 1){
		changeFrequency(octvMulti * 220.0f);
	}
	//B
	if(checkButtonPress(GPIOB, b13) == 1){
		changeFrequency(octvMulti * 246.0f);
	}



	if((checkButtonPress(GPIOB, b4) == 1) && (checkButtonPress(GPIOB, b5) == 1)){
		changeFrequencyMod(1000);
	}
